...
def get_latest_user_data():
    c.execute("SELECT user_name, password, h_schedule_time FROM users ORDER BY id DESC LIMIT 1")
    return c.fetchone()
...
# ラベルとエントリの作成
label_user_name = tk.Label(root, text="ユーザー名:")
...
if latest_user_data:
    entry_user_name.insert(0, latest_user_data[0])
    entry_password.insert(0, latest_user_data[1])
